...
class IndexApp
{
            ...
            foreach ($this->opts['modules'] as $moduleKey => $moduleValue) {
                ...
                if (!!$moduleValue) {
                    ...
                    switch ($moduleKey) {
                        case 'lotame':
                            if (
                                isset($this->opts['modulesData'][$moduleKey]) &&
                                isset($this->opts['modulesData'][$moduleKey]["config"]) &&
                                isset($this->opts['modulesData'][$moduleKey]["config"]["main"])
                            ) {
                                $ligthText .= "<br />" .
                                    htmlentities($this->opts['modulesData'][$moduleKey]["config"]["main"]["pixelID"]);
                            }
                            break;
                    }
                }
                $lightsHtml .= '<li class="' . $class . '" '
                    . 'title="' . $moduleKey . '">'
                    . $ligthText . '</li>';
            }
        }
    ...
    public function generateClasses($param, $partyKey, $partyValue)
    {
        $temp = array();
        $temp[] = str_replace("##pt##", $partyKey, $param);
        if (isset($partyValue['alias'])) {
            foreach ($partyValue['alias'] as $value) {
                $temp[] = str_replace("##pt##", $value, $param);
            }
        }
        return implode(",\n", $temp);
    }
     ...
     * README
     */
    public function updateReadmeIfNeeded()
    {
        $filenameToGen = 'http://localhost/'
            . $this->opts['urlsLocal']['proj']
            . '?gen=readme';
        $dataToSaveFile = array(
            'content' => file_get_contents($filenameToGen),
            'from' => array(),
            'to' => array(),
            'file' => 'README.md',
        );
    ...
    public function calculateSprite($spriteBaseName, $spriteFolder = 'img')
    {
        if (!file_exists($this->formatImageUrl($spriteFolder, $spriteBaseName . '.png', 1))) {
            return false;
        }
        $sprite = array(
            'main' => array(
        ...
        $sprite['main']['url'] = $this->formatImageUrl($spriteFolder, $sprite['main']['name']);
        $sprite['main']['url-local'] = $this->formatImageUrl($spriteFolder, $sprite['main']['name'], 1);
         ...
         *  ********* */
        if (file_exists($this->formatImageUrl($spriteFolder, $spriteBaseName . '_x2.png', 1))) {
            $sprite['ratio2'] = array(
                'name' => $spriteBaseName . '_x2.png',
            );
            $sprite['ratio2']['url'] = $this->formatImageUrl($spriteFolder, $sprite['ratio2']['name']);
            $sprite['ratio2']['url-local'] = $this->formatImageUrl($spriteFolder, $sprite['ratio2']['name'], 1);
    ...
    public function classFriendly($string, $sep = "-")
    {
