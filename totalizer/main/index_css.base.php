    ...
    */
    ...
    if (
        ...
        foreach ($widgetOptions["sprite"] as $spriteKey => $spriteValue) {
            $tempSprite = $IndexApp->calculateSprite($spriteValue["img"]);
            if ($tempSprite) {
                $sprites[$spriteKey] = $tempSprite["main"];
                $sprites[$spriteKey]["class"] = "div#du-body.du-body .du-" . $spriteKey;
                if (
                    isset($spriteValue["alias"]) &&
                    $spriteValue["alias"]
                ) {
                    foreach ($spriteValue["alias"] as $spriteAlias) {
                        $addClass = ",\ndiv#du-body.du-body";
                        if (!is_array($spriteAlias)) {
                            $addClass .= " " . $spriteAlias;
                        } else {
                            $addClass .= implode(" ", $spriteAlias);
                        }
                        $sprites[$spriteKey]["class"] .= $addClass;
                    }
                }
            }
            if (
                ...
                if (
                    ...
                    foreach ($spriteValue["alias"] as $spriteAlias) {
                        ...
                        $sprites_x2[$spriteKey]["class"] .= $addClass;
                    }
                }
            }
        }
        ...
        if (count($sprites) > 0) {
            foreach ($sprites as $spriteKey => $spriteValue) {
                echo $spriteValue["class"] . " {
                    background: url('" . $spriteValue["url"] . "') no-repeat scroll 999px 999px transparent;
                }\n";
            }
        }
