<script>
    ...
    var du_socialCreateBtn = function(btnType) {
        ...
        jq("a", jqNewBtn)
            ...
            .data("du-type", btnType)
            .find("span")
            .text(options.title);

        return jqNewBtn;
    };
    var du_socialUpdateBtns = function(newText, addHash) {
        socialData.message = newText;
        if (typeof addHash == "string") {
            socialData.hash += "," + du_support.camelCase(addHash);
        }
        jq(".du-social-box a", mainEl).each(function() {
            var thisType = jq(this).data("du-type");
            var newOptions = false;
            if (thisType != "Facebook") {
                newOptions = du_socialCreateBtnOptions(thisType);
            }
            if (newOptions) {
                jq(this)
                    .attr({
                        "href": newOptions.href
                    })
            }
        });
    };
